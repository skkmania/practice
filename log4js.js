 ...
 */
function Log(level,logger,prefix, options) {
 ...
 */
Log.prototype.debug3     = function(s) { if (this.getLevel()<=Log.DEBUG3) { this._log(s,"DEBUG",this); } }
Log.prototype.debug2     = function(s) { if (this.getLevel()<=Log.DEBUG2) { this._log(s,"DEBUG",this); } }
Log.prototype.debug1     = function(s) { if (this.getLevel()<=Log.DEBUG1) { this._log(s,"DEBUG",this); } }
Log.prototype.debug     = function(parent, s) { if (this.getLevel()<=Log.DEBUG) { this._log(parent, s,"DEBUG",this); } }
/**
 ...
 */
Log.prototype._log = function(parent, msg,level,obj) { 
	if (this.getPrefix()) {
		this.getLogger()(parent, this.getPrefix()+" - "+msg,level,obj); 
	} else {
		this.getLogger()(parent, msg,level,obj); 
	}
...
Log.prototype.setCSSfile = function(filename, obj) {
  //for(key in obj) alert(key + ' : ' + obj[key]);
  var header = obj._window.document.getElementsByTagName('head')[0];
  var link = obj._window.document.createElement('link');
  link.href = filename;
  link.type = "text/css";
  link.rel = "stylesheet";
...
Log.prototype.waitAsParent = function(parent){
  this.parentWaiter = parent;
}

Log.prototype.makeDiv = function(parent) {
  var ret = this._window.document.createElement('div');
  ret.className = 'logDiv';
  var parent = (this.parentWaiter ? this.parentWaiter : parent);
  if(parent)
    parent.appendChild(ret);
  else
    this._window.document.getElementById('loggerDiv').appendChild(ret);
  return ret;
}

function insertRowDiv(obj, tableDiv){
  var ret = obj._window.document.createElement('div');
  ret.className = 'logRow';
  tableDiv.appendChild(ret);
  return ret;
}

function insertCellDiv(obj, rowDiv, className){
  var ret = obj._window.document.createElement('div');
 ...
 * @param {Log} obj The originating {@link Log} object.
 */
Log.popupLogger = function(parent, msg,level,obj) {
  //for(key in obj) alert(key + ' : ' + obj[key]);
       if (obj.popupBlocker) {
	return;
       }
       if (!obj._window || !obj._window.document) {
               obj._window = window.open("",'logger_popup_window','resizable,scrollbars=yes,status=yes');
	       ...
	       if (!obj._window.document.getElementById('loggerDiv')) {
    var str = "<div width='100%' id='loggerDiv'>";
        str +=   '<div id="title" class="logRow">';
        str +=    '<div id="title_1" class="logCell" align="left">Time</div>';
        str +=    '<div id="title_2" class="logCell" width="100%" align="left">Message</div>';
        str +=  '</div></div>';
                       obj._window.document.writeln(str);
                       obj._window.document.close();
               }
       }
       var tbl = obj._window.document.getElementById("loggerDiv");
       var row = insertRowDiv(obj, parent || tbl);
       //var row = insertRowDiv(obj, tbl);

       var cell_1 = insertCellDiv(obj, row, 'logCell_1');
       var cell_2 = insertCellDiv(obj, row, 'logCell_2');
       var cell_3 = insertCellDiv(obj, row, 'logCell_3');
